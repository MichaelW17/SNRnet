...
class EfficientNetV2(nn.Module):
    ...
    def __init__(self,
                 ...
                 bias=False,
                 drop_connect_rate=0.2,  # 2023.02.23从0.2改为0
                 dropout_rate=None,
                 bn_epsilon=1e-3,
                 bn_momentum=0.01,
                 pretrained=False,
        ...
        self.head_conv = nn.Conv2d(in_channels=in_channels,
                                   ...
                                   kernel_size=1,
                                   bias=bias)
        # 2023.12.17: 添加一个可以调整输出尺寸的卷积层
        self.sinet_conv = nn.Conv2d(in_channels=head_conv_out_channels,
                                   out_channels=self.npcb * 128,
                                   kernel_size=1,
                                   bias=bias)
        self.head_bn = nn.BatchNorm2d(num_features=head_conv_out_channels,
                                      eps=bn_epsilon,
                                      momentum=bn_momentum)
        self.sinet_bn = nn.BatchNorm2d(num_features=self.npcb * 128,
                                      eps=bn_epsilon,
                                      momentum=bn_momentum)
    ...
    def forward(self, x):
        ...
        for block in self.blocks:
            x = block(x)
        # print(x.shape)  # torch.Size([1, 256, 8, 8])
        x = self.head_act(self.head_bn(self.head_conv(x)))  # torch.Size([1, 1280, 8, 8])
        x = self.head_act(self.sinet_bn(self.sinet_conv(x)))  # torch.Size([1, 6400, 8, 8])
        # print(x.shape)
        x = torch.flatten(self.avpool(x), 1)
        x = x.view(-1, self.npcb, 128)  # torch.Size([1, npcb, 128])
